...
Clinician / research dashboard for Niva (GaitGuard Nexus): live ESP32 telemetry, explainable gait analytics, IndexedDB datasets, and GitHub Pages deployment. The optional WebSocket relay lives in `relay/`.
...
Create `.env.local` in this `niva web` directory if needed:
...
Ready-to-flash firmware lives in `niva arduino/`:

- Active hardware sketch: `niva arduino/niva_hardware/niva_hardware.ino` (WiFiManager, MPU6050, OLED)
- Simpler hardcoded-WiFi sketch: `niva arduino/esp32_gaitguard_wifi_manager/esp32_gaitguard_wifi_manager.ino`

See `niva arduino/README.md` for libraries and pin map.
...
`relay/` is a small Node.js proxy used when a HTTPS dashboard (GitHub Pages) must reach an ESP32 `ws://` endpoint.
...
Default listen URL: `http://localhost:8787` with WebSocket path `/ws`. Do not commit `node_modules` or cloudflared log files.

## Deploy to GitHub Pages

This repo is configured for Pages deployment through GitHub Actions.

Workflow file (repository root):

- `.github/workflows/deploy-pages.yml` (builds this `niva web` directory)
